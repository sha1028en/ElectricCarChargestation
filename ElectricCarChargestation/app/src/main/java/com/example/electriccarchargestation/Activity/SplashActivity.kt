...
import com.example.electriccarchargestation.SplashProgressDialog
...
class SplashActivity :
    ...
    private fun initPublicData( amount: Int): MutableList<PublicData> {
        ...
        coroutineScope.launch ( exceptionHandler)  {
            ...
            if( items.size > 0) {
                ...
                runOnUiThread { this@SplashActivity.progressDialog.initProgress( 0, progressRear, 0) }

                //insert value into DB.publicdataTLB
                for( i in 0 until items.size) {
                    this@SplashActivity.dbHelper.insertPublicData( items[ i])
                    this@SplashActivity.progressDialog.addCurrentProgress()
                }
    ...
    private suspend fun setPublicData(amount: Int) = withContext( Dispatchers.IO) {
        ...
        val rd: BufferedReader = if ( conn.responseCode in 200..300)    { BufferedReader(
