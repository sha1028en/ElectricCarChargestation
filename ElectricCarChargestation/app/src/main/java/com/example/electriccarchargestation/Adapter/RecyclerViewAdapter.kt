...
class RecyclerViewAdapter(val context: Context, var publicDatalist: MutableList<PublicData>?,val selectActivity: String): RecyclerView.Adapter<RecyclerViewAdapter.RecyclerViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerViewHolder, position: Int) {
        ...
        binding.tvItemName.text = selectedPublicData?.stationName ?: "EMPTY"
        binding.tvItemAddress.text = selectedPublicData?.address  ?: "EMPTY"

        if (selectedPublicData?.bookmark == 1)  { binding.ivEmptyStar.setImageResource(R.drawable.fullstar_24) }
        else                                    { binding.ivEmptyStar.setImageResource(R.drawable.emptystar_24) }
